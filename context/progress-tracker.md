...
**2026-08-08 — BLOCK 4: MVP DOMAIN CONTRACT FROZEN.** `docs/domain/18-BLOCK4-mvp-domain-freeze.md`. Built by reading the repository rather than prior summaries. Every unresolved item in the project is now classified A–E; nothing remains as "future work."

**The audit found four live defects that would each have produced a false financial result, and two specification voids an engineer would have had to fill by inventing.** That is the value of the block, and none of them was visible from the decision register alone — they were in the documents an engineer would actually read.

⚠ **The most dangerous was the currency contradiction.** The first-release scope and the build plan both listed **multi-currency as out of scope** while D-024 makes **FX normalisation Tier 1** and D-028 already requires currency, rate and rate date on every financial event. Both readings were supported by current text, and the wrong one sat in the document read first. An engineer building single-currency from it would record USD freight as an EGP figure at that day's rate; compared across an EGP devaluation, **a premium that is entirely currency movement presents as an operational deterioration** — the exact failure D-016 was written to prevent, arriving through a scope document rather than through the saving engine. Resolved as a distinction, not a new capability: transacting is out, capture and normalisation are in — and each amount normalises **at its own effective date**, never at a single current rate.

⚠ **§4.4 was still carrying `(current price − best comparable price) × annual volume` as live text.** Part 2.3 recorded the category as superseded by Mechanism 02 and never rewrote the formula. It is a rate applied to a total — the precise error D-017 rejected and D-027 elevated — sitting in the taxonomy document under a heading that looked current. **A superseded formula with no superseded marker is worse than a wrong one, because it reads as current.** Two other documents still carried *"deduplicated by subject"*, which D-020 replaced with mechanism-level dedup precisely because one PO line can carry two independent effects; subject-level deduplication discards one and misdirects the fix.

**Two voids where the specification simply stopped.** D-012 requires the headline to be a range and **never said what its bounds are** — every available move (±20%, a confidence interval, a standard deviation) is an invented constant, in the number the whole product is judged on. Resolved as an **evidence partition**: lower bound is claims whose every input is `ACTUAL`/`CALCULATED`, upper bound adds those carrying an `ESTIMATED`/`ASSUMED` input. Both bounds are real claims; no probability appears anywhere. And nothing said what an **aggregate does with a member returning `INSUFFICIENT_DATA`** — treat it as zero and the headline silently omits it; apply contagion literally and the headline is permanently uncomputable. Contagion governs the inputs of a value; **exclusion governs the members of a set, and exclusions are disclosed.**

**`A-03` was resolved by removal.** The Saving Opportunity object carried a required `Confidence` field with no formula and a rule forbidding category constants — so an engineer would have synthesised one, most plausibly by blending gate outcomes, which is exactly the *"gates never become scores"* prohibition arriving as a UI necessity. **The MVP carries evidence strength and coverage facts stated plainly instead.** A stated coverage fact is more useful to a finance manager than a score, and it cannot be wrong.

**The reframing that made the MVP definable.** Every mechanism's *currency* is gated by a factory fact that may not exist — M01 by `F-01`, M02 by `F-12`/`F-13`, M03 by `F-31`. **None of their *correctness* is.** So the MVP is defined as producing a **defensible finding**, not a saving number. **An MVP that returns `INSUFFICIENT_DATA` for every opportunity, with each gap named, is a passing MVP.** Any other definition creates pressure to invent an input when the factory turns out not to record one — the likeliest route to a false financial result in this project.

**Scope shrank, twice, on evidence.** The MVP mechanism is M01's lead-time slice, which needs no carrying rate — so **`F-08` is not an MVP blocker**, contradicting the readiness audit's gating list. And the MVP's currency comes from procurement documents, not the cost reference — so **`N-01` and `U-14` move out of the blocking set** too. Twelve factory fields decide the MVP; eight block the build, two block only currency, two are claim gates.

**Orders & Supply Movement got a hard test rather than a restatement:** an element is a domain requirement **if and only if a locked mechanism's counterfactual, gate, offset or realization check reads it.** Thirteen elements pass. **Current location, supplier location and the map read by nothing** — legitimate product ambition, not domain necessity, and recorded as such so effort does not go there ahead of what gates mechanisms.

**`Q-07` is fixed, not deferred** — and it was one defect away from being an MVP blocker.
